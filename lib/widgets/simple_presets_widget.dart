...
class _SimplePresetsWidgetState extends State<SimplePresetsWidget> {
  ...
  List<ExpansionPanel> _buildExpansionPanels() {
    return List<ExpansionPanel>.generate(widget.presets.length, (int index) {
      return ExpansionPanel(
        headerBuilder: (BuildContext context, bool isExpanded) {
          return ListTile(title: Text('Preset ${index + 1}'));
        },
        body: Column(
            children: List<WeaponTile>.generate(widget.presets[index].weapons.length, (int i) {
