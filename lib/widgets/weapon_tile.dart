...
class WeaponTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (weapon == null) {
      return ListTile(
        title: Text(AppLocalizations.of(context).stringBy("weapons", "unknown")),
        subtitle: Text(AppLocalizations.of(context).stringBy("weapons", "no_data")),
      );
    }
    ...
    return ListTile(
      ...
      subtitle: Text(weapon!['weapon_type']),
      trailing: IconButton(
        onPressed: () {
          showDialog(
            context: context,
                  ...
                  body: SingleChildScrollView(
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          const Text("Ammos"),
                          if (weapon!['ammos'] != null && weapon!['ammos'].length == 1)
                            _buildAmmoDetails(context, weapon!['ammos'][0])
                          else if (weapon!['ammos'] != null)
                            ListView.builder(
                              shrinkWrap: true,
  ...
  Widget _buildAmmoDetails(BuildContext context, Map<String, dynamic>? ammo) {
    if (ammo == null) {
    ...
    return Column(
      ...
      children: [
        ...
        if (ammo["max_distance"] != null) Text('Max Distance: ${ammo["max_distance"]} m'),
        if (ammo["explosive_type"] != null) Text('Explosive Type: ${localize("explosives", ammo["explosive_type"])}'),
        if (ammo["explosive_mass"] != null) Text('Explosive Mass: ${ammo["explosive_mass"]} kg'),
        const Divider(),
      ],
    );
  }
